...
void exti_init()
{
    ...
    // enable input line 13 rising edge detection (falling edge is FTSR)
    EXTI->RTSR |= EXTI_RTSR_TR13_Msk;
    // don't mask interrupt request from line 13 (event is EMR)
    EXTI->IMR |= EXTI_IMR_IM13;
    // ISER[1] is 32 - (56 - 32) (since this is past ISER[0])
    NVIC->ISER[1] |= (1 << 8);
